...
public class RiderServiceTest {
  ...
  void whenGetRiderStatsWithZeroReviews_thenReturnMap() throws NonExistentResource {
    ...
    Mockito.when( orderProfitRepo.findByRider( Mockito.any() ) )
  ...
  @Test
  void whenReviewRiderUpdateRider() throws NonExistentResource {
    
    Rider riderWithReviews = new Rider( "mal@gmail.com", "Manuel Antunes", "migant", true, location, 1, 4 );
    Mockito.when( riderRepo.findById( anyLong() ) ).thenReturn( Optional.of( rider ) );
    Mockito.when( riderRepo.save( any() ) ).thenReturn( riderWithReviews );
    
    Rider updated = riderService.reviewRider( 1L, 4 );
    assertThat( updated.getSumOfReviews() ).isEqualTo( 4 );
    assertThat( updated.getNumberOfReviews() ).isEqualTo( 1 );
    
    Mockito.verify( riderRepo, Mockito.times( 1 ) ).findById( anyLong() );
    Mockito.verify( riderRepo, Mockito.times( 1 ) ).save( any() );
  }
  
  @Test
  void whenReviewRiderUpdateRiderRiderDoesntExist() throws NonExistentResource {
    
    Mockito.when( riderRepo.findById( anyLong() ) ).thenReturn( Optional.empty() );
    
    assertThrows( NonExistentResource.class, () -> riderService.reviewRider( 1L, 4 ) );
    
    Mockito.verify( riderRepo, Mockito.times( 1 ) ).findById( anyLong() );
    Mockito.verify( riderRepo, Mockito.times( 0 ) ).save( any() );
  }
